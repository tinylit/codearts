...
namespace CodeArts
{
    ...
    public class PagedList<T> : IEnumerable<T>, IEnumerable
    {
        ...
        /// 总数。
        /// </summary>
        public int Count => isEmpty ? 0 : queryable.Count();

        /// <summary>
        /// 获取迭代器。
        /// </summary>
        /// <returns></returns>
        public virtual IEnumerator<T> GetEnumerator() => isEmpty ? enumerable.GetEnumerator() : queryable.Skip(Size * Page).Take(Size).GetEnumerator();
