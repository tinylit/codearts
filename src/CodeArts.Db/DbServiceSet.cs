...
namespace CodeArts.Db
{
    ...
    public class DbServiceSet<TEntity> where TEntity : class, IEntiy
    {
        ...
        static DbServiceSet() => tableInfo = TableRegions.Resolve<TEntity>();

        /// <summary>
        /// inheritdoc
        /// </summary>
        protected DbServiceSet()
        {
        }
        ...
        /// inheritdoc
        /// </summary>
        public DbServiceSet(IDbConnectionAdapter connectionAdapter, string connectionString)
        {
        ...
        private abstract class DbRouteExecuter
        {
            ...
            public IDbRouter<TEntity> DbRouter => DbServiceRouter<TEntity>.Instance;
            ...
            private int Execute(List<Tuple<string, Dictionary<string, object>>> results, int? commandTimeout = null)
            {
                int influenceLine = 0;
                ...
                if (commandTimeout.HasValue)
                {
                    ...
                    if (isolationLevel.HasValue)
                    {
                        using (var connection = CreateDb())
                        {
                            ...
                            using (var transaction = isolationLevel == IsolationLevel.Unspecified
                                ...
                                : connection.BeginTransaction(isolationLevel.Value))
                            {
                                results.ForEach(x =>
                                {
                                    ...
                                    using (var command = connection.CreateCommand())
                                    {
                                        ...
                                        {
                                            LookupDb.AddParameterAuto(command, kv.Key, kv.Value);
                                        }
            ...
            public abstract List<Tuple<string, Dictionary<string, object>>> PrepareCommand(ICollection<TEntity> entries);
            ...
            private async Task<int> ExecutedAsync(List<Tuple<string, Dictionary<string, object>>> results, int? commandTimeout, CancellationToken cancellationToken)
            {
                int influenceLine = 0;
                        ...
                        using (var connection = CreateDb())
                        {
                            if (connection is DbConnection dbConnection)
                            {
                                ...
                                await using (var transaction = isolationLevel == IsolationLevel.Unspecified
                                     ...
                                     : await dbConnection.BeginTransactionAsync(isolationLevel.Value, cancellationToken).ConfigureAwait(false))
                                {
                                    foreach (var x in results)
                                    {
                                        ...
                                        await using (var command = dbConnection.CreateCommand())
                                        {
                                            ...
                                            influenceLine += await command.ExecuteNonQueryAsync(cancellationToken);
        ...
        private class Insertable : DbRouteExecuter, IInsertable<TEntity>
        {
            ...
            protected Tuple<string, Dictionary<string, object>> SqlGenerator(string tableName, IEnumerable<TEntity> list, Dictionary<string, string> cols)
            {
...
label_valid:
            ...
            public override List<Tuple<string, Dictionary<string, object>>> PrepareCommand(ICollection<TEntity> entries)
            {
                string tableName = tableInfo.TableName;

                if (Settings.Engine == DatabaseEngine.Oracle)
                {
                    var results = new List<Tuple<string, Dictionary<string, object>>>(entries.Count);
                ...
                {
                    var results = new List<Tuple<string, Dictionary<string, object>>>(1);
            ...
            public IInsertableByCommit<TEntity> Except<TColumn>(Expression<Func<TEntity, TColumn>> columns)
                => Except(DbServiceRouter<TEntity>.Instance.Except(columns));
            ...
            {
                SetTransaction(IsolationLevel.Unspecified);
        ...
        private class InsertableByFrom : Insertable
        {
            ...
            public override List<Tuple<string, Dictionary<string, object>>> PrepareCommand(ICollection<TEntity> entries)
            {
                ...
                else
                {
                    ...
                    var results = new List<Tuple<string, Dictionary<string, object>>>(parameterCount / MAX_PARAMETERS_COUNT + 1);
        ...
        private class InsertableByAnalyseFrom : Insertable
        {
            ...
            public override List<Tuple<string, Dictionary<string, object>>> PrepareCommand(ICollection<TEntity> entries)
            {
                ...
                {
                    var results = new List<Tuple<string, Dictionary<string, object>>>(insertColumns.Count * entries.Count / MAX_PARAMETERS_COUNT + 1);
        ...
        private class Deleteable : DbRouteExecuter, IDeleteable<TEntity>
        {
            ...
            public override List<Tuple<string, Dictionary<string, object>>> PrepareCommand(ICollection<TEntity> entries)
            {
                ...
                List<Tuple<string, Dictionary<string, object>>> results = new List<Tuple<string, Dictionary<string, object>>>(1);
            ...
            protected Tuple<string, Dictionary<string, object>> Simple(string tableName, KeyValuePair<string, string> col, IEnumerable<TEntity> entities)
            {
                var sb = new StringBuilder();
                ...
                var parameters = new Dictionary<string, object>(entities.Count());
                ...
                if (list.Count == 1)
                {
                    return new Tuple<string, Dictionary<string, object>>(sb.Append("=")
                        .Append(list[0])
                        .ToString(), parameters);
                }
                ...
                {
                    return new Tuple<string, Dictionary<string, object>>(sb.Append(" IN (")
            ...
            protected Tuple<string, Dictionary<string, object>> Complex(string tableName, Dictionary<string, string> cols, IEnumerable<TEntity> entities)
            {
                var sb = new StringBuilder();

                var parameters = new Dictionary<string, object>(entities.Count() * cols.Count);
        ...
        private class Updateable : DbRouteExecuter, IUpdateable<TEntity>
        {
            ...
            protected Tuple<string, Dictionary<string, object>> SqlGenerator(string tableName, IEnumerable<TEntity> entities, Dictionary<string, string> updateSetColumns, Dictionary<string, string> whereColumns)
            {
                int count = entities.Count();

                var sb = new StringBuilder(count * 7);
                var parameters = new Dictionary<string, object>(count * (updateSetColumns.Count + whereColumns.Count));
        ...
        private class UpdateableByFrom : Updateable
        {
            ...
            public override List<Tuple<string, Dictionary<string, object>>> PrepareCommand(ICollection<TEntity> entries)
            {
                ...
                else
                {
                    ...
                    var results = new List<Tuple<string, Dictionary<string, object>>>(entries.Count / offset + 1);
        ...
        private class UpdateableByAnalyseFrom : Updateable
        {
            ...
            public override List<Tuple<string, Dictionary<string, object>>> PrepareCommand(ICollection<TEntity> entries)
            {
                ...
                var results = new List<Tuple<string, Dictionary<string, object>>>(total / MAX_PARAMETERS_COUNT + 1);
