...
{
    /// <summary>
    /// SQL（表名称：{SELECT:yep_users}，表别名：{=user}，字段名称：[name]，字段别名：[=user_name]，参数:{name}，对象存在：&lt;YES:TABLE:yep_users&gt;`{DROP TABLE yep_users;}`，对象不存在：&lt;NO:TABLE:yep_users&gt;`{CREATE TABLE yep_users (Id int not null,name varchar(100));}`）
    /// </summary>
    ...
    public class SQL : ISQL
    {
        ...
        /// 复杂修改命令。
        /// </summary>
        private readonly static Regex PatternChangeComplex = new Regex(@"\b((?<type>delete)[\x20\t\r\n\f]+(?<alias>\w+)|(?<type>update)[\x20\t\r\n\f]+(?<alias>\w+)[\x20\t\r\n\f]+set([\x20\t\r\n\f]+[\w\.\[\]]+[\x20\t\r\n\f]*=[\x20\t\r\n\f]*[?:@]?[\w\.\[\]]+,[\x20\t\r\n\f]*)*[\x20\t\r\n\f]+[\w\.\[\]]+[\x20\t\r\n\f]*=[\x20\t\r\n\f]*[?:@]?[\w\.\[\]]+)[\x20\t\r\n\f]+from[\x20\t\r\n\f]+(\w+\.)*(?<name>\w+)((?!\b(insert|update|delete|create|drop|alter|truncate|use|set)\b)[^;]+);?", RegexOptions.IgnoreCase | RegexOptions.Compiled);

        /// <summary>
        /// 创建或删除表命令。
        /// </summary>
        private readonly static Regex PatternCreateOrDrop = new Regex(@"\b(?<type>create|drop)[\x20\t\r\n\f]+(table|view)[\x20\t\r\n\f]+(?<if>if[\x20\t\r\n\f]+(not[\x20\t\r\n\f]+)?exists[\x20\t\r\n\f]+)?(\w+\.)*(?<name>\w+)[\x20\t\r\n\f]*\(((?!\b(select|insert|update|delete|create|drop|alter|truncate|use|set)\b)[^;]+);?", RegexOptions.IgnoreCase | RegexOptions.Multiline | RegexOptions.Compiled);

        /// <summary>
        /// 查询命令。
        /// </summary>
        private readonly static Regex PatternForm = new Regex(@"\b(from|join)[\x20\t\r\n\f]+(\w+\.)*(?<name>\w+)[\x20\t\r\n\f]+(as[\x20\t\r\n\f]+)?(?<alias>(?!\b(where|on|join|group|order|having|select|(left|right|inner|outer)[\x20\t\r\n\f]+join)\b)\w+|)?([\x20\t\r\n\f]*,[\x20\t\r\n\f]*(\w+\.)*(?<name>\w+)[\x20\t\r\n\f]+(as[\x20\t\r\n\f]+)?(?<alias>(?!\b(where|on|join|group|order|having|select|(left|right|inner|outer)[\x20\t\r\n\f]+join)\b)\w+|)?)*", RegexOptions.IgnoreCase | RegexOptions.Multiline | RegexOptions.Compiled);
        ...
        /// 字段别名。
        /// </summary>
        private readonly static Regex PatternAsField = new Regex(@"\[\w+\][\x20\t\r\n\f]+as[\x20\t\r\n\f]+(?<alias>[\p{L}\p{N}@_]+)|\[\w+\][\x20\t\r\n\f]+(as[\x20\t\r\n\f]+)?(?<correct>\[(?<alias>[\p{L}\p{N}@_]+)\])|\bas[\x20\t\r\n\f]+(?<alias>(?!\bselect\b)[\p{L}\p{N}@_]+)", RegexOptions.IgnoreCase | RegexOptions.CultureInvariant | RegexOptions.Compiled);
        ...
        /// 表别名令牌。
        /// </summary>
        private readonly static Regex PatternTableAsToken = new Regex(@"\{=(?<name>\w+)\}", RegexOptions.Compiled);
        ...
        /// 字段别名令牌。
        /// </summary>
        private readonly static Regex PatternFieldAsToken = new Regex(@"\[=(?<name>[\p{L}\p{N}@_]+)\]", RegexOptions.IgnoreCase | RegexOptions.CultureInvariant | RegexOptions.Compiled);
        ...
        static SQL()
        {
            ...
            var sb = new StringBuilder()
                ...
                .Append("|,").Append(whitespace).Append(@"*((?<alias>\w+)\.)?(?<name>([_a-z]\w*))").Append(whitespace).Append(@"*[=),]") //? ,[name], ,[name]) ,[name]=[value]
        ...
        private static string MakeSQL(string sql)
        {
            ...
            //? 创建或删除表指令。
            sql = PatternCreateOrDrop.Replace(sql, item =>
            {
                ...
                var value = item.Value.Substring(nameGrp.Index - item.Index + nameGrp.Length);

                if (type == "CREATE")
                {
                    sb.Append(PatternFieldCreate.Replace(value, match =>
                    {
            ...
            string Form(Match item, string type)
            {
                ...
                var captures = aliasGrp.Captures.GetEnumerator();

                foreach (Capture capture in nameGrp.Captures)
                {
                    sb.Append(value.Substring(startIndex, capture.Index - index))
                        .Append("{")
                        .Append(type)
                        .Append("#")
                    ...
                    startIndex += capture.Index - index + capture.Length;
                    index = capture.Index + capture.Length;

                    if (captures.MoveNext() && (captures.Current is Capture cap) && cap.Length > 0)
                    {
                        sb.Append(value.Substring(startIndex, cap.Index - index))
                            .Append("{=")
                            .Append(cap.Value)
                            .Append("}");

                        startIndex += cap.Index - index + cap.Length;
                        index = cap.Index + cap.Length;
                    }
                }
            ...
            {
                Group nameGrp = item.Groups["alias"];
                Group correctGrp = item.Groups["correct"];

                string value = correctGrp.Success ? string.Concat("=", nameGrp.Value) : string.Concat("[=", nameGrp.Value, "]");
        ...
        public string ToString(ISQLCorrectSimSettings settings)
        {
            ...
            switch (settings.Engine)
            {
                ...
                case DatabaseEngine.SqlServer:
                    //? 条件删除。
                    sql = PatternDropIf.Replace(sql, item =>
                    {
                        var ifGrp = item.Groups["if"];
                        var nameGrp = item.Groups["name"];
                        var commandGrp = item.Groups["command"];

                        var value = item.Value;

                        var sb = new StringBuilder();

                        switch (commandGrp.Value.ToUpper())
                        {
                            case "TABLE":
                                sb.AppendFormat("IF EXIXSTS(select * from sysobjects where xtype='U' and name ='{0}')", nameGrp.Value);
                                break;
                            case "VIEW":
                            default:
                        ...
                        .Append("BEGIN")
                        .AppendLine()
                        .Append(value.Substring(0, ifGrp.Index - item.Index))
                        .Append(value.Substring(ifGrp.Index - item.Index + ifGrp.Length))
                        .AppendLine()
                        .Append("END GO")
                        .AppendLine();
                    ...
                    //? 条件创建。
                    sql = PatternCreateIf.Replace(sql, item =>
                    {
                        var ifGrp = item.Groups["if"];
                        var nameGrp = item.Groups["name"];
                        var commandGrp = item.Groups["command"];
                        ...
                        {
                            case "TABLE":
                                sb.AppendFormat("IF NOT EXIXSTS(select * from sysobjects where xtype='U' and name ='{0}')", nameGrp.Value);
                                break;
                            case "VIEW":
                                sb.AppendFormat("IF EXIXSTS(select * from sysobjects where xtype='V' and name ='{0}')", nameGrp.Value)
                                    .AppendLine()
                                    .Append("   DROP VIEW ")
                                    .Append(nameGrp.Value)
                                    .AppendLine()
                                ...
                                break;
                            case "FUNCTION":
                                sb.AppendFormat("IF NOT EXIXSTS(select * from sysobjects where xtype in ('FN', 'IF', 'TF') and name ='{0}')", nameGrp.Value);
                                break;
                            case "PROCEDURE":
                                sb.AppendFormat("IF NOT EXIXSTS(select * from sysobjects where xtype='P' and name ='{0}')", nameGrp.Value);
                                break;
                            case "DATABASE":
                                sb.AppendFormat("IF NOT EXIXSTS(select * from sys.databases where name='{0}')", nameGrp.Value);
                                break;
                            default:
                                throw new NotSupportedException();
                        }
                ...
                case DatabaseEngine.DB2:
                    ...
                    sql = PatternDropIf.Replace(sql, item =>
                     {
                         ...
                         {
                             case "TABLE":
                                 sb.AppendFormat("IF EXIXSTS(select * from syscat.tables where tabname='{0}')", nameGrp.Value.ToUpper());
                                 break;
                             case "FUNCTION":
                                 sb.AppendFormat("IF EXIXSTS(select * from syscat.functions where funcschema='{0}' AND funcname='{0}')", nameGrp.Value);
                                 break;
                             case "PROCEDURE":
                                 sb.AppendFormat("IF EXIXSTS(select * from syscat.procedures where procname='{0})", nameGrp.Value.ToUpper());
                                 break;
                             case "VIEW":
                             default:
                                 throw new NotSupportedException();
                    ...
                    sql = PatternCreateIf.Replace(sql, item =>
                    {
                        ...
                        {
                            case "TABLE":
                                sb.AppendFormat("IF NOT EXIXSTS(select * from syscat.tables where tabname='{0}')", nameGrp.Value.ToUpper());
                                break;
                            case "FUNCTION":
                                sb.AppendFormat("IF NOT EXIXSTS(select * from syscat.functions where funcschema='{0}' AND funcname='{0}')", nameGrp.Value);
                                break;
                            case "PROCEDURE":
                                sb.AppendFormat("IF NOT EXIXSTS(select * from syscat.procedures where procname='{0})", nameGrp.Value.ToUpper());
                                break;
                            default:
                                throw new NotSupportedException();
                        }
                ...
                case DatabaseEngine.Access:
                    ...
                    sql = PatternDropIf.Replace(sql, item =>
                    {
                        ...
                        {
                            case "TABLE":
                                sb.AppendFormat("IF EXIXSTS(select * from msysobjects where name ='{0}')", nameGrp.Value);
                                break;
                            case "VIEW":
                            default:
                                throw new NotSupportedException();
                    ...
                    sql = PatternCreateIf.Replace(sql, item =>
                    {
                        ...
                        {
                            case "TABLE":
                                sb.AppendFormat("IF NOT EXIXSTS(select * from msysobjects where name ='{0}')", nameGrp.Value);
                                break;
                            case "VIEW":
                            default:
                                throw new NotSupportedException();
                ...
                case DatabaseEngine.Sybase:
                    break;
                default:
                    if (PatternDropIf.IsMatch(sql))
                        throw new NotSupportedException($"暂不支持数据库引起({settings.Engine})的条件删除命令!");

                    if (PatternCreateIf.IsMatch(sql))
                        throw new NotSupportedException($"暂不支持数据库引起({settings.Engine})的条件创建命令!");
                    break;
            }
            ...
            //? 表别名。
            sql = PatternTableAsToken.Replace(sql, item => settings.Name(item.Groups["name"].Value));

            //? 字段名称。
            sql = PatternFieldToken.Replace(sql, item => settings.Name(item.Groups["name"].Value));

            //? 字段别名。
            sql = PatternFieldAsToken.Replace(sql, item => settings.Name(item.Groups["name"].Value));

            //? 参数名称。
            return PatternParameterToken.Replace(sql, item => settings.ParamterName(item.Groups["name"].Value));
        }
