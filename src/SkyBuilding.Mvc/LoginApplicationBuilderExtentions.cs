    ...
    public static class LoginApplicationBuilderExtentions
    {
        ...
        public static IApplicationBuilder UseJwtLogin(this IApplicationBuilder app)
        {
            ...
            })).Map("/login", builder => builder.Run(async context =>
            {
                ...
                {
                    await context.Response.WriteJsonAsync(DResult.Error("未配置登录接口!", SkyBuilding.StatusCodes.ServError));
                    return;
                }
                ...
                {
                    await context.Response.WriteJsonAsync(DResult.Error("不规范的登录接口!", SkyBuilding.StatusCodes.NonstandardServerError));
                    return;
                }
